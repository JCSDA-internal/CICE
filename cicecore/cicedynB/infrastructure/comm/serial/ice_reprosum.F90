...
! over threads and processes do not overflow for ilevel==1.
          ...
          enddo
       enddo
!       if (detailed_timing) call xicex_timer_stop('repro_sum_loopb')
      enddo
...
! subsequent levels contribute arr_max_shift digits.
   ...
   end subroutine ice_reprosum_int
...
! Local workspace
      ...
      character(len=*),parameter :: subname = '(ice_reprosum_tolExceeded)'

!-----------------------------------------------------------------------

      ice_reprosum_tolExceeded = .false.
      if (ice_reprosum_reldiffmax < 0.0_r8) return
      ...
      max_abs_diff = 0.0_r8
      do ifld=1,nflds
         if (rel_diff(1,ifld) > ice_reprosum_reldiffmax) then
      ...
      if (exceeds_limit > 0) then
         if (master) then
            write(llogunit,*) subname,trim(name), &
                            ': difference in fixed and floating point sums ', &
                            ' exceeds tolerance in ', exceeds_limit, &
                            ' fields.'
            write(llogunit,*) subname,'  Maximum relative diff: (rel)', &
                            rel_diff(1,max_rel_diff_idx), ' (abs) ', &
                            rel_diff(2,max_rel_diff_idx)
            write(llogunit,*) subname,'  Maximum absolute diff: (rel)', &
                            rel_diff(1,max_abs_diff_idx), ' (abs) ', &
                            rel_diff(2,max_abs_diff_idx)
         endif
         ice_reprosum_tolExceeded = .true.
       endif


   end function ice_reprosum_tolExceeded
...
! Local workspace
      ...
      character(len=*),parameter :: subname = '(ice_reprosum_ddpdd)'
...
! Local workspace
      ...
      real(r8) e, t1, t2
      integer i
      character(len=*),parameter :: subname = '(ice_reprosum_mod:DDPDD)'
...
! Local workspace
    ...
    character(len=*),parameter :: subname = '(ice_reprosum_mod:split_indices)'
